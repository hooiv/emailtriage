...
def format_email_html(email: dict) -> str:
    ...
    return f"""
    <div style="border:1px solid #e5e7eb;border-radius:8px;padding:12px;margin:8px 0;background:{"#f9fafb" if is_archived else "white"};">
        <div style="display:flex;justify-content:space-between;margin-bottom:8px;">
            <span style="font-weight:bold;">📧 {email["id"]}</span>
        </div>
        <div><b>From:</b> {email.get("sender_name", email.get("sender", "Unknown"))}</div>
        <div><b>Subject:</b> {email.get("subject", "No subject")}</div>
        <div style="margin:8px 0;padding:8px;background:#f3f4f6;border-radius:4px;font-size:13px;max-height:80px;overflow:hidden;">{str(email.get("body", ""))[:200]}</div>
        <div>{" ".join(tags) if tags else "Uncategorized"}</div>
    </div>
    """
...
def format_inbox_html(observation: dict) -> str:
    ...
    if not inbox:
        return '<div style="text-align:center;padding:40px;color:#6b7280;">📭 Inbox empty</div>'
    html = f'<div style="padding:10px;"><h3>📥 Inbox ({len(inbox)} emails)</h3>'
...
def take_action_ui(email_id: str, action_type: str, category: str, priority: str, reply_content: str):
    ...
    try:
        ...
        for h in reversed(action_history[-5:]):
            color = "#22c55e" if h["reward"] > 0 else "#ef4444" if h["reward"] < 0 else "#6b7280"
            history_html += f'<div style="border-left:3px solid {color};padding:4px;margin:2px 0;">{h["action"]} → {h["email"]} <span style="color:{color};">{h["reward"]:+.2f}</span></div>'
        history_html += "</div>"
...
with gr.Blocks(title="Email Triage OpenEnv") as demo:
    ...
    with gr.Row():
        with gr.Column(scale=1):
            ...
            action_btn = gr.Button("▶️ Execute")
        ...
        with gr.Column(scale=2):
            inbox_html = gr.HTML('<div style="text-align:center;padding:40px;">Click Reset to start</div>')
